...
if(!empty($_COOKIE)){
    $login = $_COOKIE['login']; 
    $spendings = R::findAll('transactions', 'user = ? ORDER BY time DESC', [$login]);
    foreach($spendings as $transaction):
...
?>
            ...
            <div class='transaction-history__item' onclick="toggleDetails(this)">
                <div class='transaction-main'>
                        ...
                        <?php
                            switch ($transaction['categorie']) {
                                ...
                                case 'Transport':
                                    ...
                                    break;
                                case 'Unknown':
                                    echo "<img src='/img/icons/warning_amber.svg' alt='unknown'>";
                                    break;
                            }
                        ?>
                    </div>
                    ...
                    <h2 class='history-item__amount red-spending-color '><?php echo '- ' . $transaction['amount'] . ' $' ?></h2>
                </div>
